...
auto evaluate_division_reminder(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for %"};
    }
...
auto evaluate_seconds_to_hours(std::stack<double>& stack) -> void
{
    if (stack.size() < 1) {
    ...
    for (auto const each : args) {
        try {
            if (each == "p") {
                ...
                evaluate_division_reminder(stack);
            } else if (each == "**") {
                evaluate_exponentiation(stack);
            } else if (each == "sqrt") {
                evaluate_square_root(stack);
            } else if (each == "h") {
                evaluate_seconds_to_hours(stack);
            } else {
