...
// auto oblicz_wyznacznik(const std::vector<int> macierz, const int
...
//            for (auto i = size_t{0}; i < macierz.size(); i++) {
//
//                if (wiersz(i, wielkosc_macierzy) != w && kolumna(i,
//                wielkosc_macierzy) != k) {
//
//                    macierz_pomniejszona.push_back(macierz[i]);
//
//                } else if (wiersz(i, wielkosc_macierzy) == w && kolumna(i,
//                wielkosc_macierzy) == k) {
//
//                    j = macierz[i];
//
...
//            oblicz_wyznacznik(macierz_pomniejszona, wielkosc_macierzy - 1);
//        }
...
//
//            for (auto j = int{0}; j < static_cast<int>(macierz_trzy.size());
//            j++) {
//
...
//            }
//
//            wyznacznik += pow((-1), (1 + kolumna(i, wielkosc_macierzy))) *
//            macierz[i] * oblicz_wyznacznik(macierz_trzy, wielkosc_macierzy-1);
//        }
//
...
auto main(int argc, char* argv[]) -> int
{
    ...
    for (auto i = size_t{0}; i < (wielkosc_macierzy * wielkosc_macierzy); i++) {
        ...
        if (losowa_macierz == true) {
            macierz_dane.push_back(losowa_wartosc());
            rozmiar_wartosci = std::to_string(macierz_dane.back()).length();

        } else {
            auto wpisana_wartosc = std::string{};

            while (true) {
                drukuj_macierz(macierz_dane,
                               wielkosc_macierzy,
    ...
    drukuj_macierz(
        macierz_dane, wielkosc_macierzy, najwiekszy_rozmiar_wartosci);

    auto czy_wiersz = bool{true};
    std::cout << wybierz_wiersz(macierz, macierz_t, czy_wiersz) << " : "
              << czy_wiersz << "\n";
